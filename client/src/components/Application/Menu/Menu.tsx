...
const Menu = () => {
  ...
  const { setToggleArtDrawer } = useDrawerStore(
    (state) => ({
  ...
  const uniqueWardCategories = [
    ...new Set(
      data.map((art: IStreetArt) => art.properties.ward).sort((a, b) => +a - +b)
    ),
  ];
  const debounceTime = 250;

  function artistChangeHandeller(
  ...
  function programChangeHandeller(
    e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>
  ) {
    setProgramSearchQuery(e.target.value);
  }

  function wardChangeHandeller(
    e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>
  ) {
  ...
  return (
    <div
      className="bg-gradient-to-r from-slate-600 via-zinc-600 
      ...
      <div className=" shadow-lg">
        ...
        >
          <IconButton
            sx={{ p: "10px" }}
            aria-label="menu"
            onClick={() => setToggleArtDrawer(true)}
          >
            <MenuIcon />
          </IconButton>
          <InputBase
